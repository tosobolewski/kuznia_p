...
void selectionSort(int *tab, int len){
	...
	int elem_i = 0;
	int wartoscWybranyElement = 0;
	int pozycjaWybranyElement = NULL;
	
	for ( int i = 0 ; i < len ; i++)
	{
	
		wartoscWybranyElement = *(tab+i);
		
		
		// szukaj najmniejszego elememtu w nieposortowanej czesci tablicy
		for (int j=i+1 ; j < len ; j++)
		{
			// jezeli znalazles element o mniejszej wartosci
			if ( *(tab +j) < wartoscWybranyElement )
			{
				// zapamietaj jego wartosc i polozenie
				wartoscWybranyElement = *(tab+j);
				pozycjaWybranyElement =  j;
			}
		}
		
		// zamien element 'i' ze znalezionym nowym najmniejszym 
		*( tab + pozycjaWybranyElement) = *(tab+i);
		*(tab+i) = wartoscWybranyElement;
...
int main()
{
	...
	selectionSort(tabTest10000, 10000);	// wywolanie funkcji sortujacej
	...
	printArrayContent(tabTest30000,10);	// wypisz zawartosc czesci tabeli na konsole
	
	c1 = clock();   					// pomiar czasu na starcie
										// processor time, value returned is expressed in clock ticks
	selectionSort(tabTest30000, 30000);	// wywolanie funkcji sortujacej
	...
	printArrayContent(tabTest30000,40);	// wypisz wyniki na konsole
	printTimeSummary(c1, c2);	
