...
int rzut(int iloscKregli){
	...
	static int licznikStrike[2] = {0};									// = 2 liczniki zmian punktacji po 'strike' /.. drugi licznik bo premia z jednego 'strike' moze zachodzic na drugi 'strike'
	static int licznikSpare[1] = {0};									// = 1 licznik zmian punktacji po 'spare'
	...
	// zapamietaj ilosc zbitych kregli w tablicy rzutow
	// rzuty z rundy extra (po rundzie 10-tej) zapamietywane jako runda 11
	
	tablica[runda][rzut] = iloscKregli;									
																		


																		
	// oblicz premie 'strike' wynikajace z poprzednich rzutów
	// premia 'strike' z pierwszego licznika [0]
	// premia 'strike' z drugiego licznika [1]
	
	if (licznikStrike[0] != 0){												// premie
		premiaStrike = premiaStrike + punkty;
		licznikStrike[0] -=1;
	}
	
	if (licznikStrike[1] != 0){												
		premiaStrike = premiaStrike + punkty;								
		licznikStrike[1] -= 1;
	}
	...
	// i zapamietaj wynik do nastepnej rundy (zmienna ststic)
	sumaPunktow = sumaPunktow + punkty + premiaStrike + premiaSpare;		// suma punktow							
	...
	if ((rzut == 0) && (punkty == 10)){										//-// jezeli 'strike' to zwiêksz licznikStrike[]
		...
		rzut = 1;
																							...
																							// uwzgledniajac dodatkowy rzut w rundzie 10
	...
	if (rundaBiezaca < 9 ){
		...
		if ((rzutBiezacy == 1) && (licznikStrike[0] == 0 ) && (licznikStrike[1] == 0) && (licznikSpare[0] == 0)){  
	...
	} else { 																				// komunikat dla rundy extra (11-tej)
			// jezeli runda 11 extra (index = 10)
			// zalicz rzuty z rundy 11 do rundy 10
			// kontynuujac numeracje z rundy 10
			if (tablica[9][0] == 10) {														// jezeli 11 runda wynika z warunku 'strike' w 10-tej
			...
			} else if ((tablica[9][0] + tablica[9][1]) == 10){								//jezeli 11 runda wynika z warunku 'spare' w 10-tej
				...
				if (rzutDodatkowaRunda == 3 ) { 
					...
					koniecRozgrywki = 1;
																							...
																							// wypisz na ekran komunikat dla rundy 11 extra
	...
	if (koniecRozgrywki){
		...
		sumaPunktow = 0;													// wyzeruj star¹  - punkty nowej rozgrywki bed¹ liczona od 0
